...
def plot_subset(df_full, plot_focus_id, focus_col, x_col, y_col1,
    ...
    `focus_id` to subset `focus_col` of the DataFrame `df`.

    Plots `x_col` as the x-axis, `y1_col` as a primary y-axis,
    and (optional) `y2_col` as a secondary y-axis.

    Adds the chart title to reflect the `focus_id`,
    total number of records in the subset,
    and the address and coordinates.
